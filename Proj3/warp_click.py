...
class Warp:
    ...
    def show_image_get_clicks(self, img):
    ...
    def computeHomography(self, src, dest):

        n = src.shape[0]
        A = []
        for i in range(n):
            A.append(self.getPartialA(src[i], dest[i]))
    ...
    def warpPerspective(self, src, H):
